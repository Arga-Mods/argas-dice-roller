...
- ***Modifiers:*** The optional modifier buttons range from -6 to +6 and can be applied cumulatively; an optional setting adds further buttons for ±10 and ±20. Alternatively, you can enter the modifier freely (up to two digits). The modifier does not affect the individual dice, only the total result (and the result of the SWADE Wild Die). The modifiers in effect are highlighted in color on the chat card.
